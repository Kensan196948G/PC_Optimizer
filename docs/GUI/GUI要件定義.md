...
- Windows 10 / 11
- PowerShell 5.1 以上
- GUI の優先実行エンジンは PowerShell 7.x (`pwsh`)
...
- PowerShell 7 実行時の日本語ログ表示を安定させるため、UTF-8 前提のラッパースクリプト経由で起動できること
...
- GUI のログ表示は ANSI 制御シーケンスを除去して扱う
- 視覚表現は PowerShell 7 優先のポップな配色を採用するが、PowerShell 5.1 でも起動自体は維持する
