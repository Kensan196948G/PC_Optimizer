...
function Assert-ContainsPattern {
    ...
    Assert-True $Name ($Content -match $Pattern) $Detail
}
...
function Assert-NotContainsPattern {
    ...
    Assert-True $Name ($Content -notmatch $Pattern) $Detail
}
...
foreach ($svc in $requiredServiceNames) {
    Assert-True ("TC-{0:D2}: unnecessaryServices に '{1}' が含まれる" -f $serviceIndex, $svc) `
        ($sourceContent -match [regex]::Escape("'$svc'")) `
        ("'{0}' がリストに見つかりません" -f $svc)
    $serviceIndex++
}

Assert-True "TC-10: Win32_StartupCommand の呼び出しが存在する" `
    ($sourceContent -match 'Win32_StartupCommand') `
    "Win32_StartupCommand が見つかりません"

Assert-True "TC-11: Get-Service の呼び出しが存在する" `
    ($sourceContent -match 'Get-Service') `
    "Get-Service が見つかりません"

Assert-True "TC-12: タスク20が Try-Step ラッパーで実装されている" `
    ($sourceContent -match 'Try-Step[\s\S]*?スタートアップ') `
    "Try-Step 'スタートアップ...' が見つかりません"

Assert-True "TC-13: Set-Service (mutating cmd) not present" `
    ($sourceContent -notmatch 'Set-Service') `
    "Set-Service detected (mutating commands are prohibited)"

Assert-True "TC-14: Stop-Service not present" `
...
$browserCases = @(
    @{ Name = "BC-01: ブラウザキャッシュタスクに Brave が含まれる"; Pattern = 'BraveSoftware'; Detail = "BraveSoftware パスが見つかりません" },
    @{ Name = "BC-02: Brave の Cache パスが含まれる"; Pattern = 'Brave-Browser.*Cache'; Detail = "Brave-Browser Cache パスが見つかりません" },
    @{ Name = "BC-03: ブラウザキャッシュタスクに Opera が含まれる"; Pattern = 'Opera Software'; Detail = "Opera Software パスが見つかりません" },
    @{ Name = "BC-04: Opera GX のパスが含まれる"; Pattern = 'Opera GX Stable'; Detail = "Opera GX Stable パスが見つかりません" },
    @{ Name = "BC-05: ブラウザキャッシュタスクに Vivaldi が含まれる"; Pattern = 'Vivaldi'; Detail = "Vivaldi パスが見つかりません" },
    @{ Name = "BC-06: Vivaldi の User Data Cache パスが含まれる"; Pattern = 'Vivaldi.*Cache'; Detail = "Vivaldi Cache パスが見つかりません" },
    @{ Name = "BC-07: 6ブラウザ対応 - タスク名に Brave が含まれる"; Pattern = 'Try-Step.*Brave'; Detail = "Try-Step のブラウザ一覧に Brave が見つかりません" }
)
...
$powerPlanCases = @(
    @{ Name = "PP-01: 電源プランが Try-Step でラップされている"; Pattern = 'Try-Step.*電源プラン'; Detail = "Try-Step '電源プラン' が見つかりません" },
    @{ Name = "PP-02: バッテリー検出コード（Win32_Battery）が存在する"; Pattern = 'Win32_Battery'; Detail = "Win32_Battery が見つかりません" },
    @{ Name = "PP-03: バランス（balanced）プランのGUIDが含まれる"; Pattern = '381b4222-f694-41f0-9685-ff5bb260df2e'; Detail = "balanced プランの GUID が見つかりません" },
    @{ Name = "PP-04: 高パフォーマンス（highPerf）プランのGUIDが含まれる"; Pattern = '8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c'; Detail = "highPerf プランの GUID が見つかりません" },
    @{ Name = "PP-05: powercfg /setactive で電源プランを適用している"; Pattern = 'powercfg.*setactive'; Detail = "powercfg /setactive が見つかりません" },
    @{ Name = "PP-06: isLaptop による分岐が実装されている"; Pattern = '\$isLaptop'; Detail = "\$isLaptop フラグが見つかりません" }
)
foreach ($case in $powerPlanCases) {
    Assert-ContainsPattern -Name $case.Name -Content $sourceContent -Pattern $case.Pattern -Detail $case.Detail
}
...
$diskCases = @(
    @{ Name = "DO-01: ディスク最適化が Try-Step でラップされている"; Pattern = 'Try-Step.*ディスクの最適化'; Detail = "Try-Step 'ディスクの最適化' が見つかりません" },
    @{ Name = "DO-02: SSD の TRIM（Optimize-Volume -ReTrim）が実装されている"; Pattern = 'Optimize-Volume.*-ReTrim'; Detail = "Optimize-Volume -ReTrim が見つかりません" },
    @{ Name = "DO-03: HDD のデフラグ（defrag）が実装されている"; Pattern = 'defrag'; Detail = "defrag コマンドが見つかりません" },
    @{ Name = "DO-04: Get-PhysicalDisk による媒体タイプ検出がある"; Pattern = 'Get-PhysicalDisk'; Detail = "Get-PhysicalDisk が見つかりません" },
    @{ Name = "DO-05: SSD/HDD 判定に MediaType プロパティを使用している"; Pattern = 'MediaType'; Detail = "MediaType プロパティが見つかりません" },
    @{ Name = "DO-06: Get-PhysicalDisk が存在しない環境の fallback がある"; Pattern = 'Get-Command.*Get-PhysicalDisk'; Detail = "Get-PhysicalDisk の有無チェックが見つかりません" }
)
foreach ($case in $diskCases) {
    Assert-ContainsPattern -Name $case.Name -Content $sourceContent -Pattern $case.Pattern -Detail $case.Detail
}
